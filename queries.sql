...
b.  List all the male actors not named Jayne who played a character named Jayne.  List them in alphabetical order by last name.  Return the exact result.
c.  Of all movies that have a lead role identified (there is an actor with Position = 1) what fraction are women? (you can compute the result from the result of the query – the actual result does not need to be done on a single row)
*/
...
/*
2.  What Sci-Fi movie  (that has an MPAA rating) has  Jewel Staite acted in?  Return an exact result set for each appearance including her first and last name, the movie, and the name of her role. Use EXISTS in the WHERE clause to identify if a movie has an MPAA rating.
*/
...
/*
3.  Analyze the relationship between gross revenue and budget.  (Beware:  the Business table has many different types of records;  not all fields are used for each record type).
	a.  Identifying ‘Flops’ (movies with big budgets and wide distribution that didn’t earn much on opening weekend). (Note:  the intermediate queries may return results that are obviously data errors… don’t worry about this unless if affects your final result)
		i.  Write a query that identifies all movies with a budget greater than $100 million in US dollars – it should return name, budget and MovieKey. When you use it later for part iii you will only use the MovieKey.
		ii. Write another query that identifies the 5 lowest gross that opened on at least 1000 screens (restrict the set to the US opening).  It should contain the name, budget and review rating.
		iii.  Connect these two queries to find the list of movies that are low gross (query ii) and also had a large budget (query i). Use a where clause with IN and a subquery based in part i.  The final result set should contain five movies with their name, opening gross and review rating.
*/
...
/*
3. 
	b. In Hollywood Economics, Edward DeVany noted that the distribution of the ratio of Gross Revenue to Budget is rather unusual. Write a query that returns the Movie Name, Budget, Gross Revenue, and the ratio of Gross Revenue to Budget.  It should be ordered by the ratio (high to low).  Restrict your analysis to movies that were dated in the 1990s, and have revenues and budgets reported in US dollars.
		i.  First write the query to find the US gross for a movie.  Since gross increases over time and there are multiple observations, the true US gross is the maximum of all values for a given MovieKey
		ii.  Write a query that returns the name of a movie and its budget restricted to the dates and currency shown above.
		iii.  Start with query ii, and use the result of query i as the right hand part of a join condition (note: you will need to give it a correlation name).  Add the computations to the field list and the ORDER BY to get the result.
		iv.  Answer the question:  What movie in the 1990s had the highest ratio of Gross Revenue to Budget?
		v. Copy the result set to Excel and make a histogram of your result (the histogram can be a bar chart or scatterplot) . What is odd about the distribution you get?
*/
...
/*
4. In the famous ‘Six Degrees of Kevin Bacon’ game your task is find an actor who is as ‘far’ from Kevin Bacon as possible, where far is measured by the minimum number of other actors needed to connect your choice to Kevin Bacon (a connection is when they appear in the same movie or show).  For instance, John Belushi is a Bacon Number 1, because he was in Animal House with Kevin Bacon.  Dan Akroyd has a Bacon number of 2 because he was in the Blues Brothers with John Belushi, but Dan Akroyd never appeared in a movie with Kevin Bacon.  It is quite hard to find actors with Bacon numbers much greater than 4. (Note:  in doing these counts you can exclude or include Kevin himself depending on what is convenient).  Also… make sure you have the right  Kevin Bacon.
	a.  How many actors have a Bacon Number of 1? (In network analysis this is referred to as Kevin Bacon’s  degree).
	b.  How does Kevin Bacon’s degree (you computed above) compare to the degree of Denzel Washington, Harrison Ford and Paul Reubens (aka Pee-Wee Herman)?
